...
func test_grid_from_seed_is_deterministic() -> void:
	var GridClass := load("res://src/world/grid.gd")
	var g_a: Object = GridClass.call("from_seed", 0xDEADBEEF, 8, 8)
	var g_b: Object = GridClass.call("from_seed", 0xDEADBEEF, 8, 8)
	for i in range(64):
...
func test_grid_bounds_checking() -> void:
	var GridClass := load("res://src/world/grid.gd")
	var g: Object = GridClass.new(4, 4, 0)
...
func test_zone_paint_and_find_zones() -> void:
	...
	var ZoneGridClass: Script = ZoneOpsClass.ZoneGrid
	var zg: Object = ZoneGridClass.new(8, 8)
	var rect: Rect2i = Rect2i(Vector2i(2, 2), Vector2i(3, 3))
	var zones: Array = ZoneOpsClass.call("paint", zg, rect, ZoneOpsClass.ZonePurpose.HEARTH)
	assert_eq(zones.size(), 1, "One zone for a single rectangle")
	var z = zones[0]
	assert_eq(z.purpose, ZoneOpsClass.ZonePurpose.HEARTH, "Zone purpose is HEARTH")
