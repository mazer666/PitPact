...
#
# PitPact — Tide culture stub.
#
# The Tide is one of the six original cultures the
...
# from `src/core` only.
class_name TideCulture
extends RefCounted

## The culture's stable id, used as the
## `Inhabitant.culture` field value and as the
## `data/cultures/tide.tres` file's `id`
## key.
const CULTURE_ID: StringName = &"tide"
...
const DISPLAY_NAME: StringName = &"CREATURE_TIDE_NAME"


## The Tide's identifying data as a plain
...
## The Tide's per-tick morale/stress bias
