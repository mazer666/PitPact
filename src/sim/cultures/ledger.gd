...
#
# PitPact — Ledger culture stub.
#
# The Ledger is one of the six original cultures the
...
# from `src/core` only.
class_name LedgerCulture
extends RefCounted

## The culture's stable id, used as the
## `Inhabitant.culture` field value and as the
## `data/cultures/ledger.tres` file's `id`
## key.
const CULTURE_ID: StringName = &"ledger"
...
const DISPLAY_NAME: StringName = &"CREATURE_LEDGER_NAME"


## The Ledger's identifying data as a plain
...
## The Ledger's per-tick morale/stress bias
