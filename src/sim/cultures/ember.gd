...
#
# PitPact — Ember culture stub.
#
# The Ember is one of the six original cultures the
...
# from `src/core` only.
class_name EmberCulture
extends RefCounted

## The culture's stable id, used as the
## `Inhabitant.culture` field value and as the
## `data/cultures/ember.tres` file's `id`
## key.
const CULTURE_ID: StringName = &"ember"
...
const DISPLAY_NAME: StringName = &"CREATURE_EMBER_NAME"


## The Ember's identifying data as a plain
...
## The Ember's per-tick morale/stress bias
